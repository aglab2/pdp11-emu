...
public enum RegMode {
    Register(0) {
        ...
        Addr apply(MemoryModel memory, RegIndex index, @Nullable Word nextWord) {
            return index.getRegisterValue(memory).toAddr();
        }
    },
    AutoInc(2){
        @Override
        Addr apply(MemoryModel memory, RegIndex index, @Nullable Word nextWord) {
            Addr addr = index.getRegisterValue(memory).toAddr();
            index.setRegisterValue(memory, addr.inc());
            return addr;
        }
    },
    AutoDec(4){
        @Override
        Addr apply(MemoryModel memory, RegIndex index, @Nullable Word nextWord) {
            Addr newAddr = index.getRegisterValue(memory).toAddr().dec();
            index.setRegisterValue(memory,newAddr);
            return newAddr;
        }
    },
    Index(6){
        ...
        Addr apply(MemoryModel memory, RegIndex index, @Nullable Word nextWord) {
            ...
            return new Addr(index.getRegisterValue(memory).value + nextWord.value);
        }
    },
    DRegister(1){
        @Override
